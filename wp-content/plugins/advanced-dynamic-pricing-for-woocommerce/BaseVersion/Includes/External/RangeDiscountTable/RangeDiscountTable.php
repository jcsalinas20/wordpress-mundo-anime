...
class RangeDiscountTable {
	...
	public function getProductTable( $product ) {
		$themeOptions   = array_replace_recursive( $this->customizer->get_theme_options(), $this->contextOptions );
		...
		if ( $contextOptions['table']['table_layout'] === self::LAYOUT_SIMPLE ) {
			...
			foreach ( $ranges as $range ) {
				...
				foreach ( array_keys( $columns ) as $key ) {
					$value       = $this->calculateColumnValueForProductVerboseTable( $key, $range, $priceProcessor, $product );
	 ...
	 * @param \WC_Product $product
	 */
	protected function fillSimpleProductTable( $table, $contextOptions, $rule, $priceProcessor, $product ) {
		$ranges = $rule->getProductRangeAdjustmentHandler()->getRanges();
	 ...
	 * @return string|null
	 */
	protected function calculateColumnValueForProductVerboseTable( $key, $range, $priceProcessor, $product ) {
		$value    = null;
		$discount = $range->getData();
		...
		switch ( $key ) {
			case 'qty':
				...
				$value = apply_filters( 'adp_verbose_discount_product_table_cell_qty', $value, $range, $product, $priceProcessor );
				break;
			case 'discount_value':
				...
				$value = apply_filters( 'adp_verbose_discount_product_table_cell_discount_value', $value, $range, $product, $priceProcessor );
				break;
			case 'discounted_price':
				...
				$value = apply_filters( 'adp_verbose_discount_product_table_cell_discounted_price', $value, $range, $product, $priceProcessor );
				break;
		}
