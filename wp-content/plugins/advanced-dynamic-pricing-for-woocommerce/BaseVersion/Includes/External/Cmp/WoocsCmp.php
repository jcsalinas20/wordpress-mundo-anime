...
class WoocsCmp {
	...
	public function modifyContext() {
		...
		if ( $this->woocs->is_fixed_enabled ) {
			$this->context->currencyController->setCustomProductPriceCallback( array( $this, 'customProductPriceCallback') );
			$this->context->currencyController->setCustomProductRegularPriceCallback( array( $this, 'customProductRegularPriceCallback') );
			$this->context->currencyController->setCustomProductSalePriceCallback( array( $this, 'customProductSalePriceCallback') );
		}
	}
	 ...
	 * @return float
	 */
	public function customProductPriceCallback( $price, $product, $currency ) {
		$priceSettings = $this->context->priceSettings;
	 ...
	 * @return float
	 */
	public function customProductSalePriceCallback( $price, $product, $currency ) {
		$priceSettings = $this->context->priceSettings;
		if ( $currency == $this->context->currencyController->getCurrentCurrency() ) {
			$tmp_val       = $this->woocs->_get_product_fixed_price( $product, null, $price, $priceSettings->getDecimals(), 'sale' );
			return $tmp_val === - 1 ? $price : $tmp_val;
		}
	 ...
	 * @return float
	 */
	public function customProductRegularPriceCallback( $price, $product, $currency ) {
		$priceSettings = $this->context->priceSettings;
		if ( $currency == $this->context->currencyController->getCurrentCurrency() ) {
			$tmp_val       = $this->woocs->_get_product_fixed_price( $product, null, $price, $priceSettings->getDecimals(), 'regular' );
			return $tmp_val === - 1 ? $price : $tmp_val;
		}
